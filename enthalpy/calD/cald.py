...
# 📁 设置你要处理的文件夹路径
root_folder = r"E:\free_energy\0_all_newpot-2\calD\enthalpy"
...
for file_path in glob.glob(os.path.join(root_folder, "**", "*.csv"), recursive=True):
    try:
        ...
        if "Smoothed_Energy" not in df.columns or "Relative_x" not in df.columns:
            print(f"⚠️ 跳过 {file_path} ：缺少所需列")
            continue

        # 提取 S1 和 S_last
        S1 = df.iloc[0]["Smoothed_Energy"]
        S_last = df.iloc[-1]["Smoothed_Energy"]
        ...
        idx_D1 = (df["Smoothed_Energy"] - D1).abs().idxmin()
        idx_D2 = (df["Smoothed_Energy"] - D2).abs().idxmin()
