...
def main():
    ...
    interface_file = r"E:\free_energy\0all_new_pot\111\cal_result\interface_55-bins.txt"
    output_dir = r"E:\free_energy\0_all_newpot-2\111\enthalpy\bins"
    ...
    # === 保存 CSV，包含均值和标准差 ===
    output_csv = os.path.join(output_dir, "avg_std_smoothed_energy_relative_to_interface.csv")
    np.savetxt(output_csv,
               np.column_stack([rel_grid, mean_smoothed, std_smoothed]),
               delimiter=',',
               header='Relative_x,Mean_Smoothed_Energy,Std_Smoothed_Energy',
               comments='')
    print(f"✅ 均值和标准差已保存: {output_csv}")

    # === 绘图，带阴影区间 ===
    plot_file = os.path.join(output_dir, "avg_smoothed_energy_plot.png")
    plot_and_save_smoothed_energy(
        rel_grid, mean_smoothed, std_smoothed,
        out_path=plot_file,
