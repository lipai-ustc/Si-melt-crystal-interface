...
# ====== Step 4: 画图函数 ======
def plot_lq6_distribution(x_grid, smoothed_lq6, frame_index, output_dir):
    plt.figure(figsize=(8, 4))
    plt.plot(x_grid, smoothed_lq6, label=f'Frame {frame_index}')
    plt.xlabel('x')
    plt.ylabel('Smoothed LQ6')
    plt.title(f'LQ6 Distribution Along x (Frame {frame_index})')
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/frame_{frame_index}.png", dpi=300)
    plt.close()

# ====== 主函数 ======
def main():
    filename = r"E:\free_energy\0all_new_pot\100\cal_result\merge.xyz"
    output_file = r"E:\free_energy\0all_new_pot\100\cal_result\interface_54.txt"
    output_plot_dir = r"E:\free_energy\0all_new_pot\100\cal_result\lq6_plots"

    # 可以在这里指定你想绘图的帧编号，例如：[0, 10, 20]
    ...
    for i, frame in enumerate(frames):
        ...
        if i in selected_frames:
            plot_lq6_distribution(x_grid, smoothed, i, output_plot_dir)
