...
def compute_main_entropy(local_density):
    """计算每个原子的主熵 main_entropy"""
    return 2.5 - math.log(local_density * lambda_db ** 3)
...
def process_frames(merge_file, volume_file, output_file):
    ...
    with open(output_file, 'w') as out:
        for m_data, m_frame, v_frame in zip(merge_headers, merge_frames, volume_frames):
            ...
            main_entropies = [
                compute_main_entropy(rho)
                for rho in v_frame['densities']
            ]

            # 写入每帧
            N = m_frame['num_atoms']
            out.write(f"{N}\n")
            props = 'Properties=id:I:1:pos:R:3:mainentropy:R:1'
            header = f"{m_data['lattice_origin']} {props}"
            if m_data['frame_info']:
                header += f" {m_data['frame_info']}"
            out.write(header + "\n")

            for base, entropy in zip(v_frame['base_lines'], main_entropies):
                free_energy_entropy = entropy
...
if __name__ == "__main__":
    process_frames(
        r"E:\free_energy\0all_new_pot\100\cal_result\merge.xyz",
        r"E:\free_energy\0all_new_pot\100\cal_result\atomvolume.xyz",
        r"E:\free_energy\0_all_newpot-2\100\entropy\pg\main-entropy.xyz"
    )
