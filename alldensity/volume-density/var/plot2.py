...
def plot_density_with_std_from_csv(data_path, output_path):
    ...
    # 读取 sigma=0.08 的平均值 + 标准差
    sigma_low = 0.08
    file_low = os.path.join(data_path, f"density_avg_std_sigma{sigma_low}.csv")
    data_low = np.loadtxt(file_low, delimiter=',', skiprows=1)
    mean_sigma_low = data_low[:, 1]
    std_sigma_low = data_low[:, 2]

    # 绘图
    fig, ax = plt.subplots(figsize=(8, 5))
    fs = 23
    plot_x_min, plot_x_max = -20, 20
    ...
    # sigma=0.8 平均值曲线
    ax.plot(rel_grid_plot, mean_sigma_std[mask], color='C0', lw=2, alpha=1, label=f"σ={sigma_std} Å")

    # sigma=0.08 平均值曲线
    ax.plot(rel_grid_plot, mean_sigma_low[mask], color='C0', lw=2, alpha=0.3, label=f"σ={sigma_low} Å")
...
if __name__ == "__main__":
    data_path = r"E:\free_energy\0all_new_pot\111\density\aim-vor-density"  # 数据文件目录
    output_path = r"E:\free_energy\0all_new_pot\111\density\aim-vor-density"  # 输出图片目录
