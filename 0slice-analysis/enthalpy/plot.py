...
# -------------------- 用户参数 --------------------
data_folder = r"E:\free_energy\MLSCAN\python_script\analysis\0slice-analysis\enthalpy\100"  # energy 的 npy 文件所在文件夹
bins = 100                     # 直方图分箱数

# ⚠️ 这里建议根据实际 energy 范围修改，比如：
energy_min, energy_max = -10.25, -8.75  # 可根据数据调整

sigma = 2                        # 高斯平滑强度
output_folder = os.path.join(data_folder, "combined_plot")
output_figure = os.path.join(output_folder, "combined_energy.png")
...
# 找到所有 *_x*_*.npy 文件（energy）
npy_files = [os.path.join(data_folder, f) for f in os.listdir(data_folder)
             if f.endswith(".npy") and "100_energy" in f]

if not npy_files:
    raise FileNotFoundError("没有找到 energy npy 文件，请检查路径！")
...
for idx, f in enumerate(sorted(npy_files)):
    try:
        energy_data = np.load(f)
        if energy_data.size == 0:
            print(f"跳过空文件: {f}")
            continue

        # ---- 归一化直方图 ----
        hist, bin_edges = np.histogram(energy_data, bins=bins,
                                       range=(energy_min, energy_max), density=True)
...
plt.ylim(0, 4)  # 可根据实际情况调整
plt.legend(fontsize=13, loc="best", frameon=False)
plt.tight_layout()
plt.savefig(output_figure, dpi=300)
plt.show()

print(f"多条归一化 Energy 曲线已绘制并保存到: {output_figure}")
